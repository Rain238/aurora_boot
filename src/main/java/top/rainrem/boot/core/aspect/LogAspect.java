package top.rainrem.boot.core.aspect;
...
public class LogAspect {
     ...
     * 切点
     */
    @Pointcut("@annotation(top.rainrem.boot.common.annotation.Log)")
    ...
    public Object doAround(ProceedingJoinPoint joinPoint, top.rainrem.boot.common.annotation.Log logAnnotation) throws Throwable {
     ...
     * @param userId        用户ID
     */
    private void saveLog(final JoinPoint joinPoint, final Exception e, Object jsonResult, top.rainrem.boot.common.annotation.Log logAnnotation, long executionTime, Long userId) {
        ...
        if (logAnnotation == null && e != null) {
            ...
            // 请求参数
            if (logAnnotation != null && logAnnotation.params()) {
                this.setRequestParameters(joinPoint, log);
            }
            // 响应结果
            if (logAnnotation != null && logAnnotation.result() && jsonResult != null) {
    ...
    public UserAgent resolveUserAgent(String userAgentString) {
        ...
        //判断是否命中缓存
        return Objects.requireNonNull(cacheManager.getCache("userAgent")).get(userAgentStringMD5, UserAgent.class);
    }
